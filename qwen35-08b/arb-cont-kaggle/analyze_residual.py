...
paths = {
    'linear-750': refs / 'linear-alpha8-750.pt',
    'linear-1000': refs / 'linear-alpha8-1000.pt',
    'mlp-500': root / 'alpha8-mlp-500.pt',
    'mlp-750': root / 'alpha8-mlp-750.pt',
}
...
for arm, path in paths.items():
    ...
    print(arm, 'raw alpha8 matches saved global stats', flush=True)

pairs = [('mlp-500', 'linear-750'), ('mlp-500', 'linear-1000'),
         ('mlp-750', 'linear-750'), ('mlp-750', 'linear-1000'),
         ('mlp-750', 'mlp-500')]
report = {}
for a, b in pairs:
    name = a + '-vs-' + b
    ...
    for dataset in per[a]:
        ...
        if dataset in ('full-val', 'lambada'):
            ...
            print(name, dataset, 'mean alpha8 %+0.6f [%+0.6f, %+0.6f]' % (d['paired_item_mean_delta'], *d['ci95']), flush=True)
