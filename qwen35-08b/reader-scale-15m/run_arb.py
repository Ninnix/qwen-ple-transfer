...
ROOT = Path('/kaggle/working/reader-arb-15m')
...
readers_manifest = find('reader-15m-shas.json')
readers = json.loads(readers_manifest.read_text())
assert readers['source_kernel'] == 'ninnix/qwen-ple-real-r1-reader-scale-15m-p100/1'
for name, meta in readers['files'].items():
    ...
    shutil.copyfile(src, dst)
    assert sha(dst) == meta['sha256'], name
print('15M reader milestone SHA verified', flush=True)

canonical = json.loads((ROOT / 'readers' / 'canonical-10m.json').read_text())
assert canonical['reader_tokens'] == 10000384 and canonical['arbiter_calibration_tokens'] == 749568
...
for name in ('arb-reader10m-arb750-749568.pt', 'eval-reader10m-arb750.json', 'checkpoints.json'):
    src = old_manifest.parent / name
    record = old['files'][name]
    assert src.stat().st_size == record['size'] and sha(src) == record['sha256']
    dst = ROOT / 'canonical' / name
    dst.parent.mkdir(exist_ok=True)
    shutil.copyfile(src, dst)
    assert sha(dst) == record['sha256']
prior = json.loads((ROOT / 'canonical' / 'checkpoints.json').read_text())
assert prior['reader_shas']['10m'] == canonical['reader_weights_sha256']
assert prior['arbitration_shas']['reader10m-arb750'] == canonical['arbiter_checkpoint_sha256']
assert old['files']['arb-reader10m-arb750-749568.pt']['sha256'] == canonical['arbiter_checkpoint_sha256']
print('10M canonical gate and reference evaluation SHA verified', flush=True)
...
linear750 = ROOT / 'canonical' / 'arb-reader10m-arb750-749568.pt'
assert sha(linear750) == canonical['arbiter_checkpoint_sha256']
load_reader(find('real-10m-r1.reader.safetensors'), canonical['reader_weights_sha256'])
...
baseline = score('reader10m-linear750', arb)
reference = json.loads((ROOT / 'canonical' / 'eval-reader10m-arb750.json').read_text())
for key in ('val_nll', 'domain_mean', 'lambada_nll', 'hs_acc', 'lambada_acc'):
    assert abs(baseline[key] - reference[key]) < 1e-5, '10M baseline drift: ' + key
print('10M canonical baseline reproduces prior P100 evaluation', flush=True)
...
reader_path = ROOT / 'readers' / 'real-15m-r1.reader.safetensors'
reader_sha = readers['files'][reader_path.name]['sha256']
gamma15 = load_reader(reader_path, reader_sha)
...
for tag, start, end in ((500, 0, 978), (750, 978, 1464)):
    ...
    for block in range(start, end):
        ...
        opt.step()
        if (block + 1) % 100 == 0:
            print('15m arb tokens', (block + 1)*512, 'ce', float(ce.detach()),
    ...
    label = 'reader15m-arb%d' % tag
    checkpoints[label] = checkpoint(label, arb, reader_sha, end*512)
    del opt
hooks.close()
arb.requires_grad_(False)
result = score('reader15m-arb750', arb)
contrast = paired(result, baseline, 'reader15m-arb750-vs-reader10m-linear750')
...
raw10 = torch.load(ROOT / 'alpha8-reader10m-linear750.pt', map_location='cpu', weights_only=False)
raw15 = torch.load(ROOT / 'alpha8-reader15m-arb750.pt', map_location='cpu', weights_only=False)
assert set(raw10) == set(raw15) == {'full-val', 'hellaswag', 'lambada', *domains}
alpha_delta = {}
for dataset in raw10:
    a = item_stats(raw15[dataset], dataset)
    b = item_stats(raw10[dataset], dataset)
    ...
    for j, key in enumerate(alpha_keys):
        ...
        alpha_delta[dataset][key] = {
            'global_delta':result['alpha8'][dataset][key] - baseline['alpha8'][dataset][key],
...
(ROOT / 'summary.json').write_text(json.dumps({'reader10m-linear750':baseline,
                                               'reader15m-arb750':result}, indent=2))
...
(ROOT / 'checkpoints.json').write_text(json.dumps({
    ...
    'reader_shas':{'10m':canonical['reader_weights_sha256'], '15m':reader_sha},
    'arbitration_shas':checkpoints, 'canonical_arbiter_sha256':canonical['arbiter_checkpoint_sha256'],
    'gamma15':gamma15, 'alpha2_frozen':config.CANON['alpha2_frozen'],
    'calibration_shas':{k:full_man['milestones'][k]['sha256'] for k in ('500','750')},
    'frozen_eval_shas':config.REVS}, indent=2))
print('15M READER ARBITRATION COMPLETE', flush=True)
