...
stage_src = (REPO / 'tools' / 'arb_stage_src.py').read_text(encoding='utf-8')
...
for c in final['cells']:
    ...
    if cid == 'm-title':
        c = dict(c)
        c['source'] = ['# Qwen3.5-0.8B Stage A2-arb: tiny dynamic memory-arbitration on frozen REAL-5M (A/B/C)\n']
        new_cells.append(c)
        continue
    new_cells.append(c)
new_cells.append({
...
nb = dict(final)
nb['cells'] = new_cells
out = REPO / 'kaggle_qwen35_08b_ple_eval_a2_arb.ipynb'
