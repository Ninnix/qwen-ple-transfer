...
OUT = r'C:\Users\NICOLO~1.DEV\AppData\Local\Temp\opencode\smoke15.txt'
...
for stream, t, data in msgs:
    if float(t) > 240:
        lines.append('[%s t=%s] %s' % (stream, t, data.encode().decode('unicode_escape', errors='replace')[:500]))
open(OUT, 'w', encoding='utf-8').write('\n'.join(lines) + '\n')
print(len(lines), 'records after t=240')
